...
public class GoldFurnace extends BlockTileEntityRotatable {
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
		switch (dropCause) {
			...
			case PROPER_TOOL:
			case SILK_TOUCH:
				return new ItemStack[]{new ItemStack(IronFurnaces.furnaceGoldIdle)};
			default:
				return null;
		}
	}
	...
	public boolean blockActivated(World world, int x, int y, int z, EntityPlayer player) {
		if (!world.isClientSide) {
			TileEntityGoldFurnace tileentitygoldfurnace = (TileEntityGoldFurnace)world.getBlockTileEntity(x, y, z);
			player.displayGUIFurnace(tileentitygoldfurnace);
		}
	...
	public static void updateFurnaceBlockState(boolean lit, World world, int x, int y, int z) {
		...
		if (tileentity == null) {
			...
			if (lit) {
				world.setBlockWithNotify(x, y, z, config.getInt("ids.goldFurnaceActiveID"));
			} else {
				world.setBlockWithNotify(x, y, z, config.getInt("ids.goldFurnaceIdleID"));
			}
	...
	public void onBlockRemoval(World world, int x, int y, int z) {
		if (!keepFurnaceInventory) {
			...
			for (int l = 0; l < tileentitygoldfurnace.getSizeInventory(); ++l) {
				ItemStack itemstack = tileentitygoldfurnace.getStackInSlot(l);
				if (itemstack != null) {
