...
public class TileEntityDiamondFurnace extends TileEntityFurnace {

	protected final int speedModifier = config.getInt("speed.diamondFurnace");
	protected final int fuelYieldModifier = (100 * config.getInt("fuelYield.diamondFurnace")) / speedModifier;
	...
	public void updateEntity() {
		...
		if (!this.worldObj.isClientSide) {
			if (this.worldObj.getBlockId(this.xCoord, this.yCoord, this.zCoord) == config.getInt("ids.diamondFurnaceIdleID") && this.currentBurnTime == 0 && this.furnaceItemStacks[0] == null && this.furnaceItemStacks[1] != null && this.furnaceItemStacks[1].itemID == Block.netherrack.id) {
				--this.furnaceItemStacks[1].stackSize;
				if (this.furnaceItemStacks[1].stackSize <= 0) {
					this.furnaceItemStacks[1] = null;
				}

				DiamondFurnace.updateFurnaceBlockState(true, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
				furnaceUpdated = true;
			}
	...
	protected void updateFurnace() {
		DiamondFurnace.updateFurnaceBlockState(this.currentBurnTime > 0, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
	}
